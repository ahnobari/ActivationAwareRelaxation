...
PROMPT_TEMPLATES = {
    ...
    "open-math": ("<|begin_of_text|>\n\nQuestion:\n{input}\n\nMy solution:\n", "{output}", "\n"), 
}


def construct_prompt(example, data_name, prompt_type, num_shots=0):
    if False and data_name in [
        "gaokao2024_I",
        "gaokao2024_II",
        "gaokao_math_qa",
        "gaokao2024_mix",
        ...
        demos = load_prompt(data_name, prompt_type, 5)
    else:
    ...
    if len(demo_prompt) == 0 or (
        False and example["gt_ans"] not in ["A", "B", "C", "D", "E"]
    ):
        full_prompt = context
    else:
        if prompt_type == "qwen25-math-cot":
